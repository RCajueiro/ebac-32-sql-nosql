...
select count(f.film_id) as "Quantidade de Filmes", count(distinct fc.category_id) as "Quantidade de Categorias"
from film f
...
select count(distinct a.actor_id) as "Quantidade de Atores em Filmes"
from actor a
inner join film_actor fa
on a.actor_id = fa.actor_id

-- Faça uma consulta que retorne a quantidade de atores vinculados a filmes com duração maior que 2h
select count(distinct a.actor_id) as "Quantidade de Atores em Filmes com mais de 2h"
from actor a
inner join film_actor fa
on a.actor_id = fa.actor_id
inner join film f 
